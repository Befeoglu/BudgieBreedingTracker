/*
  # Notification improvements and egg fixes

  1. New Tables
    - Fixed existing policy conflicts for notifications table
    - Fixed status check constraint for eggs table
    - Added mother_id and father_id references

  2. Security
    - Fixed policy conflicts
    - Optimized RLS permissions
*/
...
-- RLS policies (create new policies with unique names)
CREATE POLICY "Users can view notifications" ON notifications FOR SELECT TO public USING (user_id = auth.uid());
CREATE POLICY "Users can insert notifications" ON notifications FOR INSERT TO public WITH CHECK (user_id = auth.uid());
CREATE POLICY "Users can update notifications" ON notifications FOR UPDATE TO public USING (user_id = auth.uid());
CREATE POLICY "Users can delete notifications" ON notifications FOR DELETE TO public USING (user_id = auth.uid());

CREATE POLICY "Users can view notification_settings" ON notification_settings FOR SELECT TO public USING (user_id = auth.uid());
CREATE POLICY "Users can insert notification_settings" ON notification_settings FOR INSERT TO public WITH CHECK (user_id = auth.uid());
CREATE POLICY "Users can update notification_settings" ON notification_settings FOR UPDATE TO public USING (user_id = auth.uid());
