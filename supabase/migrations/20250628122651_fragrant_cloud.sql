/*
  # Notification System and Clutch Enhancements
  ...
  2. Table Modifications
    - Add parent bird references to clutches table
    - Update eggs status constraint for Turkish values
    - Add unique constraint for eggs number per clutch
...
-- Add unique constraint to notification_settings if it doesn't exist
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.table_constraints
    WHERE table_name = 'notification_settings' AND constraint_name = 'notification_settings_user_id_key' AND table_schema = 'public'
  ) THEN
